...
sub loadGFF
  {
	...
	if($fields[2]=~/exon/ || $fields[2]=~/CDS/)
	  ...
	  {
	    my ($startCodon,$stopCodon,$strand,$transcriptId)=($1-1,$2,$3,$4,);
    ...
    else {@$transcripts=sort {$a->{readOrder}<=>$b->{readOrder}} @$transcripts}
    return $transcripts;
  }
#--------------------------------------------------------------------------
#   $reader->doNotSortTranscripts();
...
sub adjustStartCodons
{
	...
	if($strand eq "+")
	{
		...
		if(defined($transcript->{startCodon}) && 
		   exonContainsPoint($exon,$transcript->{startCodon})) {last}
	    }
	}
	else # $strand eq "-"
	{
