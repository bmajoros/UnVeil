...
my $tigrscanFiles=getFiles($tigrscan);
my $cdnaFiles=getFiles($cdna);
my $numTigrScan=@$tigrscanFiles;
...
my $numFiles=min($numTigrScan,$numCdna);

open(PIPE,"grep source-version $tigrscan/* |") || die;
my $line=<PIPE>;
close(PIPE);
...
print STDOUT "         |SN  SP   ACC|SN  SP |SN  SP |SN  SP |GENES\n";

evaluateExons($programName,$tigrscanFiles,$numFiles,$cdnaFiles,$tigrscan,
	      $cdna);
evaluateNuc($programName,$tigrscanFiles,$numFiles,$cdnaFiles,$tigrscan,
	    $cdna);
evaluateGenes($programName,$tigrscanFiles,$numFiles,$cdnaFiles,$tigrscan,
	      $cdna);
...
sub evaluateNuc
  {
    ...
    my $sensitivity=int(100*$TP/($TP+$FN)+0.5);
    my $specificity=int(100*$TP/($TP+$FP)+0.5);
    my $accuracy=int(100*($TP+$TN)/
		     ($TP+$TN+$FP+$FN)+0.5);
    pad(\$sensitivity,2);
    pad(\$specificity,3);
    pad(\$accuracy,2);
    print STDOUT "$label|$sensitivity\% $specificity\% $accuracy\%";
    print STDOUT "$out";
  }
#----------------------------------------------------------------
...
sub evaluateExons
  {
    ...
    $out.="|$spliceSens\% $spliceSpec\%|$codonSens\% $codonSpec\%|$exonSensitivity\% $exonSpecificity\%";
  }
#----------------------------------------------------------------
sub evaluateGenes
  {
    ...
    my $percent=int(100*$totalIdentical/$numCdnaGenes+0.5);
    pad(\$percent,2);
